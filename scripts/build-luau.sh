...
local function wrap(name, fn)
	if raw_exports[name] then
		rt_export_map[name] = fn
	end
end

wrap("_initialize", function()
	return call(raw_exports._initialize)
end)

wrap("malloc", function(size)
...
wrap("b3d_create_world", function(gravityX, gravityY, gravityZ)
...
wrap("b3d_destroy_world", function(worldHandle)
	return call(raw_exports.b3d_destroy_world, worldHandle)
end)

wrap("b3d_step", function(worldHandle, timeStep, subStepCount)
	return raw_exports.b3d_step[1](raw_exports.b3d_step, worldHandle, into_bits_f32(timeStep), subStepCount, done)
end)

wrap("b3d_create_body", function(worldHandle, bodyType, x, y, z, qx, qy, qz, qw)
...
wrap("b3d_destroy_body", function(bodyHandle)
	return call(raw_exports.b3d_destroy_body, bodyHandle)
end)

wrap("b3d_create_sphere", function(bodyHandle, radius, density, friction, restitution)
...
wrap("b3d_create_capsule", function(bodyHandle, ax, ay, az, bx, by, bz, radius, density, friction, restitution)
...
wrap("b3d_create_box", function(bodyHandle, hx, hy, hz, density, friction, restitution)
...
wrap("b3d_get_body_linear_velocity", function(bodyHandle, outPtr)
...
wrap("b3d_set_body_linear_velocity", function(bodyHandle, x, y, z)
